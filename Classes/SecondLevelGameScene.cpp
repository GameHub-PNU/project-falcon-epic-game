...
void SecondLevelGameScene::addPlaneAnimation()
{
	auto animation = Animation::create();
	animation->addSpriteFrameWithFile("SecondBackgroundAnimation/PlaneCrush1.png");
	animation->addSpriteFrameWithFile("SecondBackgroundAnimation/PlaneCrush2.png");
	animation->addSpriteFrameWithFile("SecondBackgroundAnimation/PlaneCrush3.png");
	animation->addSpriteFrameWithFile("SecondBackgroundAnimation/PlaneCrush4.png");
	animation->addSpriteFrameWithFile("SecondBackgroundAnimation/PlaneCrush5.png");
	animation->addSpriteFrameWithFile("SecondBackgroundAnimation/PlaneCrush6.png");
...
void SecondLevelGameScene::initPlanePhysicsBody()
{
...
void SecondLevelGameScene::addBackground()
{
...
void SecondLevelGameScene::initListener()
{
	auto listener = EventListenerMouse::create();
	listener->onMouseDown = CC_CALLBACK_1(SecondLevelGameScene::onMouseDown, this);
	listener->onMouseUp = CC_CALLBACK_1(SecondLevelGameScene::onMouseUp, this);
...
void SecondLevelGameScene::initGameBound()
{
...
void SecondLevelGameScene::initPlaneAndBackgroundPosition()
{
	...
	background_sprite_->runAction(Sequence::create(DelayTime::create(1.0f), CallFunc::create(CC_CALLBACK_0(SecondLevelGameScene::turnOnMusic, this)),
		DelayTime::create(2.0f), CallFunc::create(CC_CALLBACK_0(SecondLevelGameScene::initListener, this)), background_motion, nullptr));
}

void SecondLevelGameScene::initClouds()
{
...
void SecondLevelGameScene::addPlaneAndCloudsCollision()
{
	auto contactListener = EventListenerPhysicsContact::create();
	contactListener->onContactBegin = CC_CALLBACK_1(SecondLevelGameScene::onCollision, this);
	this->getEventDispatcher()->addEventListenerWithSceneGraphPriority(contactListener, this);
}

void SecondLevelGameScene::initLevelProgressBar()
{
...
void SecondLevelGameScene::updateProgressBar(float dt)
{
	level_progress_bar_->setPercent(++current_percent_);
}

void SecondLevelGameScene::turnOnMusic()
{
     sound_id_ = experimental::AudioEngine::play2d("backgroundSongLevel2.mp3", true, SettingsHandler::getSoundVolume());
	 SettingsHandler::setCurrentAudioId(sound_id_);
}

Sprite* SecondLevelGameScene::createCloud(Vec2 coordinates)
{
	...
	physics_cloud->setVelocity(Vec2(-0.115f * Director::getInstance()->getWinSize().width, 0));
...
void SecondLevelGameScene::goToPauseScene(Ref* pSender)
{
	...
	auto scene = PauseGameScene::createScene(2);
	Director::getInstance()->pushScene(scene);
}

void SecondLevelGameScene::goToGameOverScene(Ref* pSender)
{
	auto scene = GameOverScene::createScene(current_percent_, 2);
	Director::getInstance()->replaceScene(TransitionFade::create(0.5f, scene));
}

void SecondLevelGameScene::onMouseDown(EventMouse* event)
{
	physics_plane_->setDynamic(false);
...
void SecondLevelGameScene::onMouseUp(EventMouse* event)
{
	physics_plane_->setDynamic(false);
	Director::getInstance()->getRunningScene()->getPhysicsWorld()->setGravity(Vec2(0, -100));
	physics_plane_->setDynamic(true);
...
bool SecondLevelGameScene::onCollision(PhysicsContact& contact)
{
	...
	if (((1 == bodyA->getCollisionBitmask()) && (2 == bodyB->getCollisionBitmask())) || ((2 == bodyA->getCollisionBitmask()) && (1 == bodyB->getCollisionBitmask())))
	{
		if (is_game_started_)
		{
			plane_sprite_->runAction(Sequence::create(plane_crush_, CallFuncN::create(CC_CALLBACK_1(SecondLevelGameScene::goToGameOverScene, this)), nullptr));
...
bool SecondLevelGameScene::init()
{
	...
	auto pauseMenuItem = MenuItemImage::create("PauseButton.png", "PauseButton.png", CC_CALLBACK_1(SecondLevelGameScene::goToPauseScene, this));
	...
	this->scheduleUpdate();
	this->schedule(schedule_selector(SecondLevelGameScene::updateProgressBar), level_time_duration_in_seconds_ / 100.0f);
	return true;
}

void SecondLevelGameScene::update(float delta)
{
	if (is_game_started_)
	{
		...
		else
		{
			...
			plane_sprite_->runAction(Sequence::create(MoveTo::create(3, Vec2(Director::getInstance()->getWinSize().width,
				Director::getInstance()->getWinSize().height / 2.0f)), CallFuncN::create(CC_CALLBACK_1(SecondLevelGameScene::goToGameOverScene, this)), nullptr));
		}
	}
}
